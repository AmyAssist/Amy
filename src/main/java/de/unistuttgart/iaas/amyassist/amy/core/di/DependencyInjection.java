...
public class DependencyInjection implements ServiceLocator {
	...
	public synchronized void register(Class<?> cls) {
		...
		if (annotation == null) throw new ClassIsNotAServiceException(cls);
		...
		for (Class<?> serviceType : serviceTypes) {
			if (this.hasServiceOfType(serviceType)) throw new DuplicateServiceException();
		}

		if (!this.constructorCheck(cls)) throw new RuntimeException(
	...
	private boolean hasServiceOfType(Class<?> serviceType) {
		if (this.register.containsKey(serviceType)) return true;
		if (this.externalServices.containsKey(serviceType)) return true;
		return false;
	}
	...
	private <T> T get(Class<T> serviceType, Stack<Class<?>> stack,
		...
		if (this.externalServices.containsKey(serviceType)) return (T) this.externalServices.get(serviceType);
	...
	private <T> T resolve(Class<T> serviceClass, Stack<Class<?>> stack,
			Map<Class<?>, Object> resolved) {
		if (resolved.containsKey(serviceClass)) return (T) resolved.get(serviceClass);
		if (stack.contains(serviceClass)) throw new RuntimeException("circular dependencies");
	...
	private Class<?> getRequired(Class<?> serviceType) {
		if (!this.register.containsKey(serviceType)) throw new ServiceNotFoundException(serviceType);
